...
const CheckSlice = createSlice({
    ...
    reducers :{
        ...
        acceptance : (state, action) =>{ //수락버튼 누르면 작동되는 로직(화면에서 없어지고 데이터가 승인완료된 목록으로 넘어감)
            const acceptItem = state.requiredList.find(item=>item.submitId===action.payload);
            if (acceptItem) {
                state.acceptanceList = [...state.acceptanceList, acceptItem];
                state.requiredList = state.requiredList.filter(item => item.submitId !== action.payload);
